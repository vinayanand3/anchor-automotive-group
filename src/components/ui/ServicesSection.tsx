...
export function ServicesSection() {
  return (
    <section id="services" className="py-28 relative bg-transparent border-t border-black/[0.08]">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-6">
          <div className="space-y-3 max-w-2xl">
            <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-white border border-black/[0.08] text-xs font-mono tracking-widest text-[#5A606D] uppercase shadow-sm">
              <Layers className="w-3.5 h-3.5 text-[#0F1115]" />
              <span>// CORE ENGINEERING SERVICES</span>
            </div>
            <h2 className="text-3xl sm:text-4xl lg:text-5xl font-bold text-[#0F1115] tracking-tight">
              Integrated engineering for <span className="italic font-normal text-[#5A606D]">mission-critical</span> mobility.
            </h2>
          ...
          <div className="text-xs font-mono text-[#717682] bg-white px-3.5 py-2 rounded-xl border border-black/[0.06] shadow-sm">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-8">
          {disciplines.map((item, index) => (
            <motion.div
              ...
              className="p-8 sm:p-10 rounded-[2.5rem] bg-white text-[#0F1115] border border-black/[0.08] shadow-[0_20px_50px_rgba(0,0,0,0.05)] hover:shadow-xl transition-all duration-300 flex flex-col justify-between group"
            >
              <div>
                <div className="flex items-center justify-between mb-6">
                  <span className="text-xs font-mono uppercase tracking-widest text-[#717682] font-semibold">
                    {item.id} / {item.tag}
                  </span>
                  <div className="w-10 h-10 rounded-full bg-[#F6F5F2] border border-black/5 flex items-center justify-center text-[#0F1115] group-hover:rotate-12 transition-transform shadow-sm">
                    <ArrowUpRight className="w-4 h-4" />
                  </div>
                </div>

                <h3 className="text-2xl font-bold text-[#0F1115] tracking-tight mb-3">
                  {item.title}
                </h3>
              ...
              <div className="space-y-2 pt-6 border-t border-black/[0.06]">
                {item.metrics.map((metric, i) => (
                  <div key={i} className="flex items-center gap-2.5 text-xs text-[#1F232B] font-mono">
